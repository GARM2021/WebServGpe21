        ...
        <div class="breadcomb-list">
            <div class="row">
                ...
                <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 text-right" style="display: none">
                    <a href="{{ asset('predial') }}" class="btn btn-success"><i class="notika-icon notika-search"></i>
                        Nueva Consulta</a>
                </div>
                ...
                <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 text-right">
                    @if (count($cuenta->adeudos) > 0)
                        <button type="button" onclick="imprimir('{{ $info->exp }}')" id="btnImprimir"
                            class="btn btn-warning"><i class="notika-icon notika-print"></i> Imprimir</button>
                    @endif
                </div>
                        ...
                        <div class="breadcomb-ctn" style="width: 100%;"><br><br>
                            @if (count($cuenta->adeudos) > 0)
                                <div class="table-responsive">
                                    <table
                                        class="table table-condensed table-striped table-bordered table-responsive tablaCuenta">
                                        <thead>
                                            <tr>
                                        ...
                                        <tbody>
                                            ...
                                            @foreach ($cuenta->adeudos as $rowAdeudo)
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="text-right">
                                                        {{ number_format($rowAdeudo['montoimp'], 2) }}</td>
                                                    <td class="text-right">{{ number_format($rowAdeudo['saldo'], 2) }}
                                                    </td>
                                                    {{-- <td class="text-right">{{ number_format($rowAdeudo["bonImp"],2) }}</td> --}}
                                                    <td class="text-right">
                                                        {{ number_format($rowAdeudo['bonImpI'], 2) }}</td>
                                                    {{-- <td class="text-right">{{$bonImp5}}</td> --}}

                                                    <td class="text-right">
                                                        {{ number_format($rowAdeudo['salsub'], 2) }}</td>
                                                    <td class="text-right">
                                                        {{ number_format($rowAdeudo['recargos'], 2) }}</td>
                                                    <td class="text-right">
                                                        {{ number_format($rowAdeudo['bonRec'], 2) }}</td>
                                                    {{-- <td class="text-right">{{ number_format($rowAdeudo["neto"],2) }}</td> --}}
                                                    <td class="text-right">{{ number_format($rowAdeudo['netoI'], 2) }}
                                                    </td>
                                ...
                                <table class="table table-striped">
                                    ...
                                    <tr>
                                        ...
                                        <th colspan="2" class="text-right">
                                            NOTA: SE APLICA UN DESCUENTO DEL (15% ENERO, 10% FEBRERO Y 5% MARZO),
                                            ADICIONAL UN SUBSIDIO DEL 10% SOBRE EL INCREMENTO DEL IMPUESTO PREDIAL.
                                        </th>
            ...
            <div class="breadcomb-list">
                <div class="row">
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center">
                        ...
                        <img src="{{ asset('tmpl/visa.jpg') }}" style="width: 100%; max-width: 220px;"
                            alt="">
                        <br><br>
                        <form id='FormPagoLinea' method="post"
                            action="https://www.adquiramexico.com.mx/clb/endpoint/gGuadalupe">
                            <input type="hidden" name="s_transm" value="{{ $cuenta->comorder_id }}">
                            <input id="ic_referencia" type="hidden" name="c_referencia"
                                value="{{ trim($info->exp) }}">
                            ...
                            <button class="btn btn-info" id="btnPagarLinea">PAGO EN LINEA prueba &nbsp;&nbsp;&nbsp;<i
                                    class="fa fa-angle-double-right" style="color: #eea236"></i></button>
                        </form>
                        <br><br><br>
                    </div>
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center">
                        ...
                        <div class="breadcomb-wp">
                            <div class="breadcomb-ctn" style="width: 100%;"><br><br>
                                <div class="table-responsive">
                                    <table class="table table-condensed table-striped table-bordered">
                                        <tr>
                                            <th class="text-right">Impresión para pago en tiendas de
                                                conveniencia<br><br>
                                                <img src="{{ asset('paynet_logo.png') }}" style="height: 30px;"
                                                    alt="Paynet">
                                                <img src="{{ asset('7eleven.png') }}" style="height: 30px;"
                                                    alt="7 Eleven">
                                                <img src="{{ asset('extra.png') }}" style="height: 30px;"
                                                    alt="Extra"><br><br>
                                                <img src="{{ asset('farmacia_ahorro.png') }}" style="height: 30px;"
                                                    alt="Farmacias del Ahorro">
                                                <img src="{{ asset('benavides.png') }}" style="height: 30px;"
                                                    alt="Farmacias Benavides">
                                            </th>
                                            <td class="text-right"><br>
                                                <button class="btn btn-info" id="btnPaynet"
                                                    onclick="imprimirPaynet('{{ trim($info->exp) }}')"><i
                                                        class="notika-icon notika-print"></i> Imprimir Paynet</button>
                                            </td>
                                        </tr>
                                        <tr>
                                            <th class="text-right">Impresión para pago en OXXO<br><br>
                                                <img src="{{ asset('logo-oxxo.jpg') }}" style="height: 30px;"
                                                    alt="OXXO">
                                            </th>
                                            <td class="text-right"><br>
                                                <button class="btn btn-info" id="btnOxxo"
                                                    onclick="imprimirOxxo('{{ trim($info->exp) }}')"><i
                                                        class="notika-icon notika-print"></i> Imprimir OXXO</button>
                                            </td>
                                        </tr>
                                        <tr>
                                            <th class="text-right">Impresión para pago en Banco Azteca<br><br>
                                                <img src="{{ asset('logo_azteca.jpg') }}" style="height: 30px;"
                                                    alt="Banco Azteca">
                                                <img src="{{ asset('logo_elektra.jpg') }}" style="height: 30px;"
                                                    alt="Elektra">
                                            </th>
                                            <td class="text-right"><br>
                                                <button class="btn btn-info" id="btnAzteca"
                                                    onclick="imprimirAzteca('{{ trim($info->exp) }}')"><i
                                                        class="notika-icon notika-print"></i> Imprimir Banco
                                                    Azteca</button>
                                            </td>
                                        </tr>
                                        <tr>
                                            <th class="text-right">Impresión para pago en HSBC<br><br>
                                                <img src="{{ asset('logo_hsbc.jpg') }}" style="height: 30px;"
                                                    alt="Banco HSBC">
                                            </th>
                                            <td class="text-right"><br>
                                                <button class="btn btn-info" id="btnHSBC"
                                                    onclick="imprimirHSBC('{{ trim($info->exp) }}')"><i
                                                        class="notika-icon notika-print"></i> En mantenimiento</button>
                                            </td>
                                        </tr>
                                        <tr>
                                            <th class="text-right">Impresión para pago en BBVA20220523<br><br>
                                                <img src="{{ asset('logo_bbva.jpg') }}" style="height: 30px;"
                                                    alt="Banco BBVA">
                                            </th>
                                            <td class="text-right"><br>
                                                <button class="btn btn-info"
                                                    id="btnBBVA onclick="imprimirBBVA('{{ trim($info->exp) }}')"><i
                                                        class="notika-icon notika-print"></i> En mantenimiento</button>
                                            </td>
                                        </tr>
                                    </table>
